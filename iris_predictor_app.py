...
@st.cache_resource
def load_resources():
    # The .pkl files are expected to be in the same directory as app.py in the deployment environment
    try:
        model = joblib.load('best_knn_model.pkl')
        label_encoder = joblib.load('label_encoder_iris_species.pkl')
    except FileNotFoundError as e:
        st.error(f"Model or label encoder files not found: {e}. Make sure 'best_knn_model.pkl' and 'label_encoder_iris_species.pkl' are in the same directory as the app.")
        st.stop()
